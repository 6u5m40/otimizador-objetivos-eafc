(function () {
  ...
  async function _readCompletions() {
    ...
    for (let i = 0; i < 3; i++) {
      try {
        const r = await fetch("/api/fut/gg-club/", { headers, credentials: "include" });
    ...
    for (const it of items) {
      ...
      for (let i = 0; i < 3 && !settled; i++) {
        try {
          ...
          settled = true;
        } catch (e) { if (i === 2) { fail++; settled = true; } else await new Promise((res) => setTimeout(res, 500)); }
      }
    }
  ...
  async function _writeFutbin(items) {
    try {
      ...
      for (const it of items) {
        ...
        const cat = FB[slug];
        if (cat == null || !it.campaignId || !it.challengeId) { skipped++; continue; }
        ...
        for (let i = 0; i < 2 && !ok; i++) {
          try { const r = await fetch(url, { method: "POST", credentials: "include", headers: { "X-Requested-With": "XMLHttpRequest" } }); if (r.status >= 200 && r.status < 300) { okc++; ok = true; } else { fail++; ok = true; } }
          catch (e) { if (i === 1) { fail++; ok = true; } else await new Promise((res) => setTimeout(res, 400)); }
        }
      }
      return { ok: true, okc, fail, skipped };
    } catch (e) { return { ok: false, error: String(e) }; }
  }
  // sincroniza com o FUTBIN (modo "extensão → FUTBIN"): marca o que está
  // concluído e ainda não foi enviado, e DESMARCA o que foi enviado antes mas
  // não está mais concluído. Guarda por challengeId os dados p/ desmarcar depois.
  async function pushFutbin(objs) {
    const pushed = userState.futbinPushed || (userState.futbinPushed = {});
    // concluídos agora (challengeId -> {campaignId, categorySlug})
    const doneNow = {};
    for (const o of objs) { if (o.challengeId && o.campaignId && userState.done[o.id]) doneNow[o.challengeId] = { campaignId: o.campaignId, categorySlug: o.categorySlug }; }
    const items = [];
    for (const cid in doneNow) { if (!pushed[cid]) items.push({ campaignId: doneNow[cid].campaignId, challengeId: +cid, categorySlug: doneNow[cid].categorySlug, done: true }); }
    for (const cid in pushed) { if (!doneNow[cid]) { const p = pushed[cid] || {}; items.push({ campaignId: p.campaignId, challengeId: +cid, categorySlug: p.categorySlug, done: false }); } }
    ...
    const r = await runInFutgg(tabId, _writeFutbin, [items]);
    ...
    return r || { okc: 0, fail: items.length };
  }

  async function syncNow() {
    const mode = (userState.sync || {}).mode;
    if (mode === "off") { setSyncStatus(T("sync_off_hint")); return; }
    // FUTBIN: só envia (marca concluído). É a fonte da verdade a extensão.
    if (mode === "to_futbin") {
      setSyncStatus(T("sync_running"));
      const r = await pushFutbin(allObjectives());
      if (r && r.none) setSyncStatus(T("futbin_none"));
      else if (r && r.ok !== false) setSyncStatus(T("futbin_result", { n: r.okc || 0 }) + (r.fail ? " · " + T("futbin_fail", { n: r.fail }) : "") + (r.skipped ? " · " + r.skipped + " sem mapa" : ""));
    ...
    const completed = new Set((resp.objectiveChallenges || []).map(Number));
    ...
    const completedGroups = new Set((resp.objectives || []).map(Number));
    ...
    if (mode === "to" || mode === "both") {
      // 1) Empurra os objetivos REALMENTE concluídos (userState.done — não os
      //    que só aparecem feitos por causa do grupo).
      const toPush = objs.filter((o) => o.challengeId && userState.done[o.id] && !completed.has(o.challengeId)).map((o) => ({ challengeId: o.challengeId, done: true }));
      if (toPush.length) { await runInFutgg(tabId, _writeCompletions, [toPush]); pushed = toPush.length; }
      // 2) Empurra a conclusão do GRUPO (campanha) para o FUT.GG. O FUT.GG NÃO
      //    completa o grupo sozinho ao marcar os objetivos — precisa deste write
      //    (objectType "OBJECTIVE", objectId = eaId da campanha).
      const byCamp = {};
      ...
      for (const cid in byCamp) {
        const items = byCamp[cid];
        const req = groupRequired(items);
        const indDone = items.filter((o) => userState.done[o.id]).length;
        const localComplete = gdMap[cid] || (items.length > 0 && indDone >= req);
    ...
    if (created) chrome.tabs.remove(tabId);
    saveState(); render();
    if (pushed > 0 || pushedGroups > 0) reloadFutggTabs(); // reflete no FUT.GG na hora
    ...
    if (pulled === 0 && removed === 0 && completed.size > 0 && withId === 0) {
      ...
      setSyncStatus(T("sync_result", { pulled, pushed }) + (pushedGroups ? " (+" + pushedGroups + " grupos)" : "") + (removed ? " · desmarcados " + removed : "") + "  · lidos " + completed.size + " · com id " + withId + (groupsPulled ? " · grupos " + groupsPulled : ""));
    }
  }
